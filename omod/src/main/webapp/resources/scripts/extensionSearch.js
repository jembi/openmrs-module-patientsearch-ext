function doNav(patientId){
	...
	//alert(ctx);
	document.location.href = ctx + '/patientDashboard.form?patientId='+patientId;
}
...
function assess(id){
	...
	if(patientSearchId == ''){
		...
		document.getElementById('searchString').innerHTML = "Viewing results for '<strong>" + document.getElementById("patientSearch").value + "</strong>'";
...
for (var i = 1, row; row = table.rows[i]; i++) { 	
	...
	DWRMyModuleService.filter(idString, moreThanAgeString, lessThanAgeString, gender, dob, province, district, sectors, cells, villages, obj);
	...
	function obj(patientList){
	    ...
	    //
		   ...
		   var noPages;
		    ...
		    if(patientList.length > paginationId) {
		    	//alert(patientList.length + "  "+ paginationId)
		    ...
		    if(pageCheck < 5){
		      ...
		      if(noPages >= 1){
		    	 ...
		    	 for(var i =noPages; i>=1; i--){
		    	  // pageString =  document.getElementById('pages').value;
		    	   if(pageString == undefined){
		    		   pageString = "";
		    	   }
		    	   
		    	   
		    	  var link =  '<li><a href="Javascript:assess('+i+');" onclick="activate(this)">'+ i +'</a></li>';
		    	   //var link =  '<li><a href="Javascript:assess('+i+'); " >'+ i +'</a></li>';
		      ...
		      if(pageString != undefined){
		    	  ...
		    	  document.getElementById('pages').innerHTML = "<ul id ='pagination-digg'>" + pageString+"</ul>";
		      document.getElementById('pageInfo').innerHTML = "Showing " +no+ " to " +newNo+ " of "+  patientList.length +" entries";
		      
		      }else{
		    	 // document.getElementById('pages').innerHTML = "";
		    	  document.getElementById('pageInfo').innerHTML = "Showing " +1+ " to " +patientList.length+ " of "+  patientList.length +" entries";
		      }	
		    }
	    ...
	    if(patientList.length == 0){
	    ...
	    else if(patientList.length != 0 && (paginationId != ''|| paginationId != undefined)) {
	    	var counter;
	    	if(paginationId >= patientList.length){
	        ...
	        for( var i=0; i<counter;i++)
	        {
	        	...
	        	if(i%2 == 0){
	        		paginatedBody += '<tr class = "point" id="'+ i + '"style="background-color: #E6E6FA" onclick="doNav('+patientList[i].patientId+');" onMouseover="highlight(this);" onMouseout="reColour(this);"  >';
	        	}else{
	        		paginatedBody += '<tr class = "point" id="'+i+'" onclick="doNav('+patientList[i].patientId+');"  onMouseover="highlight(this);" onMouseout="reColourWhite(this);" >';
	        	}        	
	        	...
	        	paginatedBody += '</td>'
	        		...
	        		paginatedBody += '</td>'
	                    ...
	                    if(patientList[i].stateProvince == undefined){
	    	            	...
	    	            	if(patientList[i].village == undefined || patientList[i].village == null){
	    ...
	    if(patientList.length == 0)
	        document.getElementById('results').innerHTML = theader + tbody + tfooter;
	        else
	        document.getElementById('results').innerHTML = theader + paginatedBody + tfooter;
	}
...
function objects(patientList){
    ...
    if(pageCheck < 5){
      ...
      document.getElementById('pageInfo').innerHTML = "Showing " +no+ " to " +newNo+ " of "+  patientList.length +" entries";
    	  ...
    	  var no = 1;
    	 // document.getElementById('pages').innerHTML = "ZZZZ";
    	  document.getElementById('pageInfo').innerHTML = "Showing " +no+ " to " +patientList.length+ " of "+  patientList.length +" entries";
      }
    ...
    {
    	if(i%2 == 0){
    		tbody += '<tr class = "point" id="'+ i + '"style="background-color: #E6E6FA" onclick="doNav('+patientList[i].patientId+');" onMouseover="highlight(this);" onMouseout="reColour(this);"  >';
    	}else{
    		tbody += '<tr class = "point" id="'+i+'" onclick="doNav('+patientList[i].patientId+');"  onMouseover="highlight(this);" onMouseout="reColourWhite(this);" >';
    	}
    ...
    if(patientList.length != 0 && (paginationId != ''|| paginationId == undefined)) {
    	var counter;
    	if(paginationId >= patientList.length){
    		counter = patientList.length;
    	}else if(paginationId < patientList.length) {
    ...
    document.getElementById('resultsBackup').innerHTML = tbkupheader + tbody + tfooter;
